    ...
    while breadth.size():
        ...
        if node not in visited:
            ...
            if node == end:
                index = 1
                for value in path:
                    for direction in roomGraph[value][1]:
                        if index == len(path):
                            world.printRooms()
                            print(path)
                            print(directions)
                            return directions
                        if roomGraph[value][1][direction] == path[index]:
                            directions.append(direction)
                            index+=1
            
            for connected_room in roomGraph[node][1].values():
                new_path = path[:]
...
treasureMap = {0: [(3, 5), {'n': 1, 's': 5, 'e': 3, 'w': 7}], 1: [(3, 6), {'s': 0, 'n': 2}], 2: [(3, 7), {'s': 1}], 3: [(4, 5), {'w': 0, 'e': 4}], 4: [(5, 5), {'w': 3}], 5: [(3, 4), {'n': 0, 's': 6}], 6: [(3, 3), {'n': 5, 'w': 11}], 7: [(2, 5), {'w': 8, 'e': 0}], 8: [(1, 5), {'e': 7}], 9: [(1, 4), {'n': 8, 's': 10}], 10: [(1, 3), {'n': 9, 'e': 11}], 11: [(2, 3), {'w': 10, 'e': 6}]}
...
def connect(d, roomA, roomB):
    treasureMap_final[roomA][d] = roomB
    treasureMap_final[roomB][opposite(d)] = roomA
...
def question_mark(dictionary):
    for rValue in dictionary:
        if dictionary[rValue] == '?':
            print('YEEEEEEEEES!')
            return True
    print('NOOOOOOOOOO!')
    return False
...
def map_room(d, roomA, roomB):
    add_room(roomB, treasureMap_final)
    connect(d, roomA, roomB)





# print('HELP', player.currentRoom)

add_room(player.currentRoom.id, treasureMap_final)



def explore_final(starting_room):
#     """
...
#     """
    ...
    unexplored_room = None

    while len(treasureMap_final) < len(roomGraph):
        room = player.currentRoom.id
        print(f'current room is: {room}.')

        

        # if we haven't treasureMap_final this room yet, mark it as treasureMap_final
        if room not in treasureMap_final:
            add_room(room, treasureMap_final)

        questions = ['?' for direction in treasureMap_final[room] if treasureMap_final[room][direction] == '?' and direction in treasureMap_final[room] ]

        print('beginning', len(questions), room )

        # pick a random exit from the current room
        nextDirection = None
        prevDirection = traversalPath[-1] if len(traversalPath) > 0 else None
        if prevDirection in treasureMap_final[room] and treasureMap_final[room][prevDirection] == '?':
            nextDirection = prevDirection
        else:
            for direction in treasureMap_final[room]:
                if treasureMap_final[room][direction] == '?':
        ...
        if nextDirection is None:
            ...
            depth.push(opposite(nextDirection))
            treasureMap_final[room][nextDirection] = player.currentRoom
        ...
        #     map_room(direction, room, player.currentRoom.id)
        #     print('tester', treasureMap_final)
        #     if treasureMap_final[room][direction] not in visited:
        #         depth.push(treasureMap_final[room][direction])
...
explore_final(player.currentRoom.id)
