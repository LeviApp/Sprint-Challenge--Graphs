...
roomGraph={0: [(3, 5), {'n': 1, 's': 5, 'e': 3, 'w': 7}], 1: [(3, 6), {'s': 0, 'n': 2, 'e': 12, 'w': 15}], 2: [(3, 7), {'s': 1}], 3: [(4, 5), {'w': 0, 'e': 4}], 4: [(5, 5), {'w': 3}], 5: [(3, 4), {'n': 0, 's': 6}], 6: [(3, 3), {'n': 5, 'w': 11}], 7: [(2, 5), {'w': 8, 'e': 0}], 8: [(1, 5), {'s': 9, 'e': 7}], 9: [(1, 4), {'n': 8, 's': 10}], 10: [(1, 3), {'n': 9, 'e': 11}], 11: [(2, 3), {'w': 10, 'e': 6}], 12: [(4, 6), {'w': 1, 'e': 13}], 13: [(5, 6), {'w': 12, 'n': 14}], 14: [(5, 7), {'s': 13}], 15: [(2, 6), {'e': 1, 'w': 16}], 16: [(1, 6), {'n': 17, 'e': 15}], 17: [(1, 7), {'s': 16}]}
...
# simplified_maze = {0: {1,3,5,7}, 1: {0,2,12,15}, 2: {1}, 3: {0,4}, 4: {3}, 5: {0,6}, 6: {5,11}, 7: {0, 8}, 8:{7,9}, 9:{8,10}, 10:{9,11},  11:{6,10}, 12:{1,13}, 13:{12,14}, 14:{13}, 15:{1,16}, 16:{15,17}, 17:{16}}
...
#         visited.append(node)
#         if len(visited) > 1:
#             if visited[-1] not in simplified_maze[visited[-2]]:
#                 path = visited[:]
#                 visited.pop()
#                 visited.pop()
#                 visited.extend(bfs(path[-2],path[-1]))
#                 print('test', visited)
...
#         for connected_room in simplified_maze[node]:
#             print('connected_room', connected_room, 'node', node)
#             if connected_room not in visited and connected_room not in depth.stack:
#                 depth.push(connected_room)
...
def dft(starting_vertex):
    ...
    while depth.size():
        ...
        visited.append(node)
        if len(visited) > 1:
            if visited[-1] not in set(roomGraph[visited[-2]][1].values()):
                path = visited[:]
                visited.pop()
                visited.pop()
                visited.extend(bfs_numbers(path[-2],path[-1]))
                print('test', visited)




        for connected_room in set(roomGraph[node][1].values()):
            print('connected_room', connected_room, 'node', node)
            if connected_room not in visited and connected_room not in depth.stack:
                depth.push(connected_room)
    ...
    for value in visited:
            for direction in roomGraph[value][1]:
                try: 
                    if roomGraph[value][1][direction] == visited[index]:
